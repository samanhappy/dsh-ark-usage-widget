...
- **宽侧边栏**：整行显示火山引擎标志 + `66.10% · 49.30% · 24.70%`（近 5 小时 / 近一周 / 近一月，两位小数），位置在「设置」操作栏**下方**
- **窄 rail（折叠态）**：只显示火山引擎标志
- **图标**：官方 Volcengine 品牌标志（volcengine.com 站点 SVG 的路径数据内联），`currentColor` 填充（宽栏 16px / rail 18px），随主题与 hover 变色，并与右侧数字视觉中线对齐
...
- 侧边栏图标使用火山引擎品牌标志（商标归火山引擎所有），此处仅用于标识对应的云服务；不能用于暗示本插件与火山引擎有官方关联。
